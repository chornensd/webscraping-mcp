...
```
mcp/
├── mcp_server.js          MCP 入口：scrape_page / debug_page / scrape_books / scrape_quotes / get_scrape_health
├── src/
...
│   ├── policies/          robots / security(SSRF) / rate_limit / behavior / proxy（代理池）
│   ├── scrapers/          示例与回归抓取器（books/quotes）
│   └── utils/             logger / output / stealth / humanize / metrics（指标）
